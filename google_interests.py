...
async def summarize_google(user_input, user_profile, last_3_conversations):
    ...
    system_message = f"Bob, as an AI, you are about to decide what overall sentence to Google for the user. Considering the latest text message from the user: \"{user_input}\", the last 6 conversations: \"{last_3_conversations}\" and the user profile: \"{user_profile}\"\nWhat is the most relevant short google search, that most likely will have results, to continue this conversation? Google Search:"
